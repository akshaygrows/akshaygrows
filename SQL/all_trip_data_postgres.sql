WITH base AS (
         SELECT ops_main.awb,
            ...
            ops_main.shipment_status,
	 		ops_main.rto_initiated_time,
            date_trunc('day'::text, ops_main.pickuptime) AS pickup_date,
                CASE WHEN date_trunc('day', ops_main.delivertime) <= ops_main.edd 
					and ops_main.shipment_status <> 'Cancelled'
					and (ops_main.rto_initiated_time > edd or  ops_main.rto_initiated_time is null) 
					THEN 1 ELSE 0 END AS otd,
                CASE
                ...
                END AS datetime,
                CASE WHEN date_trunc('day', ops_main.first_ofd_time) <= ops_main.edd 
					and ops_main.shipment_status <> 'Cancelled'
					and (ops_main.rto_initiated_time > edd or  ops_main.rto_initiated_time is null) 
					THEN 1 ELSE 0 END AS ofd,
            ops_main.edd,
            ...
            tracking_events.rto_ofd_time::date AS rto_ofd_date,
                CASE
                    ...
                    WHEN ops_main.shipment_status = ANY (ARRAY['Reached Nearest Hub'::text, 'Reached Destination City'::text, 'In-Transit']) THEN 'fresh_attempt'::text
          ...
          WHERE 1 = 1 AND ops_main.shipping_partner = 'Hyperlocal'::text AND ops_main.created_date >= (date_trunc('month'::text, now() + '05:30:00'::interval) - '2 months'::interval) AND ops_main.shipping_city IS NOT NULL AND n.node_name::text <> 'GSCAN TEST'::text
        ), missing AS (
         ...
         SELECT base.shipping_city,
            ...
            sum(
                CASE
                    ...
                    ELSE 0
                END) AS delivered,
            count(case when shipment_status <> 'Cancelled'
					and (rto_initiated_time > edd or rto_initiated_time is null) then awb else null end) AS efd,
